...
function gather_and_setup () {
    ...
    DEST_RUN_LIST_DESCRIPTION="${RESULTS_DIRECTORY}/run_list_description.csv"
    if [ ! -f "${DEST_RUN_LIST_DESCRIPTION}" ]; then
    	cp "${RUN_LIST_DESCRIPTION}" "${DEST_RUN_LIST_DESCRIPTION}"
    fi  
}
...
mv logs/* "${TARGET_LOG_DIRECTORY}/"
mv results/* "${TARGET_RESULTS_DIRECTORY}/"
